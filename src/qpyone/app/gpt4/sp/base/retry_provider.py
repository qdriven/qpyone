...
from qpyone.app.gpt4.sp.base.base_provider import BaseShuffleRetryProvider
from qpyone.app.gpt4.sp.base.errors import RetryProviderError, \
    RetryNoProviderError
from qpyone.app.gpt4.sp.base.schemas import *


class RetryProvider(BaseShuffleRetryProvider):
    """
    ...
    def create_completion(
        ...
        **kwargs
        ...
        """
        providers = [p for p in self.providers if
                     stream and p.supports_stream] if stream else self.providers
        if self.shuffle:
        ...
        for provider in providers:
            ...
            try:
                ...
                for token in provider.create_completion(model, messages, stream,
                                                        **kwargs):
                    yield token
                    started = True
                if started:
    ...
    def raise_exceptions(self) -> None:
        ...
        if self.exceptions:
            raise RetryProviderError("RetryProvider failed:\n" + "\n".join([
                f"{p}: {exception.__class__.__name__}: {exception}" for p, exception in
                self.exceptions.items()
            ]))
