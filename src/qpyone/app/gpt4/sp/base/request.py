...
try:
    ...
    from .requests.curl_cffi import StreamResponse, StreamSession

    has_curl_cffi = True
except ImportError:
    import curl_cffi.requests.session as session
    from .requets.aiohttp import StreamResponse, StreamSession
...
def get_args_from_browser(url: str, webdriver: WebDriver = None, proxy: str = None,
...
def get_session_from_browser(url: str, webdriver: WebDriver = None, proxy: str = None,
                             timeout: int = 120) -> session.Session:
    ...
    # )
    return session.Session(**args, proxies={"https": proxy, "http": proxy},
