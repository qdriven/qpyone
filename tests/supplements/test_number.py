...
def test_ordinal(test_input: str, expected: str) -> None:
    assert supplements.ordinal(test_input) == expected
...
def test_intcomma(
    ...
    assert supplements.intcomma(*test_args) == expected


def test_intword_powers() -> None:
    # make sure that powers & human_powers have the same number of items
    assert len(supplements.number.powers) == len(supplements.number.human_powers)
...
def test_intword(test_args: list[str], expected: str) -> None:
    assert supplements.intword(*test_args) == expected
...
def test_apnumber(test_input: int | str, expected: str) -> None:
    assert supplements.apnumber(test_input) == expected
...
def test_fractional(test_input: float | str, expected: str) -> None:
    assert supplements.fractional(test_input) == expected
...
def test_scientific(test_args: list[typing.Any], expected: str) -> None:
    assert supplements.scientific(*test_args) == expected
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([1, supplements.intword, 1e6, None, "under "], "under 1.0 million"),
        ([math.nan], "NaN"),
        ([math.inf], "+Inf"),
        ([-math.inf], "-Inf"),
    ],
...
def test_clamp(test_args: list[typing.Any], expected: str) -> None:
    assert supplements.clamp(*test_args) == expected
...
def test_metric(test_args: list[typing.Any], expected: str) -> None:
    assert supplements.metric(*test_args) == expected
