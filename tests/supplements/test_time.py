...
from qpyone.supplements import time
...
# Tests for the public interface of supplements.time


def nd_nomonths(d: dt.timedelta) -> str:
    return supplements.naturaldelta(d, months=False)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (dt.timedelta(days=500), "1 year, 4 months"),
        (dt.timedelta(days=365 * 2 + 35), "2 years"),
        # regression tests for bugs in post-release supplements
...
def test_naturaldelta(test_input: int | dt.timedelta, expected: str) -> None:
    assert supplements.naturaldelta(test_input) == expected
...
def test_naturaltime(test_input: dt.datetime, expected: str) -> None:
    assert supplements.naturaltime(test_input) == expected


def nt_nomonths(d: dt.datetime) -> str:
    return supplements.naturaltime(d, months=False)
...
def test_naturalday(test_args: list[typing.Any], expected: str) -> None:
    assert supplements.naturalday(*test_args) == expected
...
def test_naturaldate(test_input: dt.date, expected: str) -> None:
    assert supplements.naturaldate(test_input) == expected
...
def test_naturaldelta_minimum_unit_default(seconds: float, expected: str) -> None:
    ...
    # Act / Assert
    assert supplements.naturaldelta(delta) == expected
...
def test_naturaldelta_minimum_unit_explicit(
    ...
    # Act / Assert
    assert supplements.naturaldelta(delta, minimum_unit=minimum_unit) == expected
...
def test_naturaltime_minimum_unit_default(seconds: float, expected: str) -> None:
    ...
    # Act / Assert
    assert supplements.naturaltime(datetime) == expected
...
def test_naturaltime_minimum_unit_explicit(
    ...
    # Act / Assert
    assert supplements.naturaltime(datetime, minimum_unit=minimum_unit) == expected
...
def test_precisedelta_one_unit_enough(
    ...
    assert supplements.precisedelta(val, minimum_unit=min_unit) == expected
...
def test_precisedelta_custom_format(
    ...
    assert supplements.precisedelta(val, minimum_unit=min_unit, format=fmt) == expected
...
def test_precisedelta_suppress_units(
    ...
    assert (
        supplements.precisedelta(val, minimum_unit=min_unit, suppress=suppress)
        == expected
    )


def test_precisedelta_bogus_call() -> None:
    assert supplements.precisedelta(None) == "None"

    with pytest.raises(ValueError):
        supplements.precisedelta(1, minimum_unit="years", suppress=["years"])

    with pytest.raises(ValueError):
        supplements.naturaldelta(1, minimum_unit="years")
