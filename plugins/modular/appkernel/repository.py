...
from .model import CustomProperty
from .model import Expression
from .model import Index
from .model import Model
from .model import Property
from .model import SortOrder
...
class Query(object):
        ...
        if isinstance(where, Expression):
            if isinstance(where.lhs, (Property, CustomProperty)):
                if where.lhs.backreference.within_an_array:
                    ...
                    self.filter_expr[
                        str(where.lhs.backreference.array_parameter_name)
                    ] = where.ops.lmbda(
                        (
                            where.lhs.backreference.parameter_name,
                    ...
                    self.filter_expr[
                        str(where.lhs.backreference.parameter_name)
                    ] = where.ops.lmbda(Query.__extract_rhs(where.rhs))
            elif isinstance(where.lhs, Expression) and isinstance(
                where.rhs, Expression
            ):
    ...
    def __xtract_expression(self, expression: Expression):
        ...
        if isinstance(expression.lhs, Property):
            ret_val.append(
                {
                    expression.lhs.backreference.parameter_name: expression.ops.lmbda(
                        Query.__extract_rhs(expression.rhs)
                    )
                }
            )
...
class MongoQuery(Query):
    def __init__(
        self, connection_object: pymongo.collection.Collection, user_class, *expressions
    ):
    ...
    def find(self, page: int = 0, page_size: int = 100) -> Model:
        ...
        if len(self.sorting_expr) == 0:
            cursor = (
                self.connection.find(self.filter_expr)
                .skip(page * page_size)
                .limit(page_size)
            )
        else:
    ...
    def find_one_and_update(self, **update_expression):
        ...
        hit = self.connection.find_one_and_update(
            self.filter_expr, upd, return_document=ReturnDocument.AFTER
        )
        return (
            Model.from_dict(
                hit,
...
class Repository(object):
    ...
    @classmethod
    def find_by_query(
        cls, query={}, page=1, page_size=50, sort_by=None, sort_order=SortOrder.ASC
    ):
        """
...
class MongoRepository(Repository):
        ...
        if issubclass(cls, Model):
            ...
            for key, value in cls.__dict__.items():
                if isinstance(value, Property):
                    if value.index:
                        fct = index_factories.get(
                            value.index, MongoRepository.not_supported
                        )
                        fct(
                            cls.get_collection(),
                            key,
                            value.index.sort_order
                            if hasattr(value.index, "sort_order")
                            else SortOrder.ASC,
                        )
    ...
    def version_check(required_version_tuple):
        ...
        if current_version < required_version_tuple:
            raise AppKernelException(
                "This feature requires a min version of: {}".format(
                    ".".join(required_version_tuple)
                )
            )
    ...
    def prepare_document(document, object_id=None):
        if isinstance(document, Model):
            ...
            document = Model.to_dict(
                document, convert_id=True, converter_func=mongo_type_converter_to_dict
            )
    ...
    @classmethod
    def patch_object(cls, document, object_id=None):
        return cls.__save_or_update_dict(
            document, object_id=object_id, insert_if_none_found=False
        )

    @classmethod
    def __save_or_update_dict(
        cls, document, object_id=None, insert_if_none_found: bool = True
    ):
        has_id, document_id, document = MongoRepository.prepare_document(
            document, object_id
        )
        if has_id:
            update_result = cls.get_collection().update_one(
                {"_id": document_id}, {"$set": document}, upsert=insert_if_none_found
            )
            db_id = update_result.upserted_id or (
                document_id if update_result.matched_count > 0 else None
            )
        else:
    ...
    @classmethod
    def save_object(
        cls, model: Model, object_id: str = None, insert_if_none_found: bool = True
    ) -> object:
        assert model, "the object must be handed over as a parameter"
        assert isinstance(model, Model), "the object should be a Model"
        document = Model.to_dict(
            model, convert_id=True, converter_func=mongo_type_converter_to_dict
        )
    ...
    def replace_object(cls, model: Model):
        ...
        update_result = cls.get_collection().replace_one(
            {"_id": document_id}, document, upsert=False
        )
        return (
            (update_result.upserted_id or document_id)
    ...
    def find_by_query(
        ...
        if sort_by:
            py_direction = (
                pymongo.ASCENDING if sort_order == SortOrder.ASC else pymongo.DESCENDING
            )
    ...
    def update_many(cls, match_query_dict, update_expression_dict):
        ...
        """
        update_result = cls.get_collection().update_many(
            match_query_dict, update_expression_dict
        )
        return update_result.modified_count
    ...
    def aggregate(cls, pipe=[], allow_disk_use=True, batch_size=100):
        cursor = cls.get_collection().aggregate(
            pipe, allowDiskUse=allow_disk_use, batchSize=batch_size
        )
...
class AuditableRepository(MongoRepository):
    ...
    def save_object(cls, model: Model, object_id=None):
        ...
        if has_id:
            ...
            update_result = cls.get_collection().update_one(
                {"_id": doc_id}, upsert_expression, upsert=True
            )
