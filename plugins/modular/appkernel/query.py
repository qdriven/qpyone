...
class QueryProcessor(object):
    ...
    def create_key_from_instance_method(provisioner_method):
        if isinstance(provisioner_method, types.FunctionType) and hasattr(
            provisioner_method, "inner_function"
        ):
            return "{}_{}".format(
                provisioner_method.inner_function.__self__.__name__,
                provisioner_method.__name__,
            )
        elif isinstance(provisioner_method, types.FunctionType):
            return "{}".format(provisioner_method.__name__)
        else:
            return "{}_{}".format(
                provisioner_method.__self__.__name__, provisioner_method.__name__
            )
    ...
    def get_query_param_names(provisioner_method):
        ...
        return request_set.difference(
            appkernel.service.qp.reserved_param_names.get(
                QueryProcessor.create_key_from_instance_method(provisioner_method)
            )
        )
