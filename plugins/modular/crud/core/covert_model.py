...
def convert_table_to_model(db_model):
    ...
    table_dict = {"__table__": db_model, "__tablename__": db_name}

    if not db_model.primary_key:
        table_dict["__mapper_args__"] = {"primary_key": [i for i in db_model._columns]}
        NO_PRIMARY_KEY = True
    ...
    return (
        type(f"{db_name}DeclarativeBaseClass", (declarative_base(),), table_dict),
        NO_PRIMARY_KEY,
    )
