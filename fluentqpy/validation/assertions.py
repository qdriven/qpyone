...
class AssertionBuilder(object):
        ...
        if len(items) == 0:
            ...
            if missing:
                if self._check_dict_like(self.val, return_as_bool=True):
                    ...
                    self._err('Expected <%s> to contain items %s, but did not contain %s.' % (
        ...
        if len(items) == 0:
            ...
            for i in items:
                if i in self.val:
                    found.append(i)
            if found:
                self._err('Expected <%s> to not contain items %s, but did contain %s.' % (
        ...
        if len(items) == 0:
            ...
            for i in self.val:
                if i not in items:
                    extra.append(i)
            if extra:
                self._err('Expected <%s> to contain only %s, but did contain %s.' % (
            ...
            for i in items:
                if i not in self.val:
                    missing.append(i)
            if missing:
                self._err('Expected <%s> to contain only %s, but did not contain %s.' % (
                    self.val, self._fmt_items(items), self._fmt_items(missing)))
        return self
    ...
    def is_greater_than(self, other):
        ...
        if self.val <= other:
            if type(self.val) is datetime.datetime:
                self._err('Expected <%s> to be greater than <%s>, but was not.' % (
    ...
    def is_less_than(self, other):
        ...
        if self.val >= other:
            if type(self.val) is datetime.datetime:
                self._err('Expected <%s> to be less than <%s>, but was not.' % (
                    self.val.strftime('%Y-%m-%d %H:%M:%S'), other.strftime('%Y-%m-%d %H:%M:%S')))
            else:
        ...
        if self.val < low or self.val > high:
            if val_type is datetime.datetime:
                self._err('Expected <%s> to be between <%s> and <%s>, but was not.' % (
                    self.val.strftime('%Y-%m-%d %H:%M:%S'), low.strftime('%Y-%m-%d %H:%M:%S'),
    ...
    def is_close_to(self, other, tolerance):
        ...
        if self.val < (other - tolerance) or self.val > (other + tolerance):
            if type(self.val) is datetime.datetime:
                ...
                self._err('Expected <%s> to be close to <%s> within tolerance <%s>, but was not.' % (
    ...
    def is_not_close_to(self, other, tolerance):
        ...
        if self.val >= (other - tolerance) and self.val <= (other + tolerance):
            if type(self.val) is datetime.datetime:
                ...
                self._err('Expected <%s> to not be close to <%s> within tolerance <%s>, but was.' % (
                    self.val, other, tolerance))
        return self
        ...
        if hasattr(self.val, 'keys') and callable(getattr(self.val, 'keys')) and hasattr(self.val, '__getitem__'):
            ...
            if missing:
                self._err('Expected <%s> to be subset of %s, but %s %s missing.' % (
                    self.val, self._fmt_items(superdict), self._fmt_items(missing),
            ...
            if missing:
                self._err('Expected <%s> to be subset of %s, but %s %s missing.' % (
                    self.val, self._fmt_items(superset), self._fmt_items(missing),
                    'was' if len(missing) == 1 else 'were'))
    ...
    def contains_value(self, *values):
        ...
        for v in values:
            if v not in self.val.values():
                missing.append(v)
        if missing:
            self._err('Expected <%s> to contain values %s, but did not contain %s.' % (
        ...
        if len(values) == 0:
            ...
            for v in values:
                if v in self.val.values():
                    found.append(v)
            if found:
                self._err('Expected <%s> to not contain values %s, but did contain %s.' % (
    ...
    def contains_entry(self, *args, **kwargs):
        ...
        if missing:
            self._err('Expected <%s> to contain entries %s, but did not contain %s.' % (
    ...
    def does_not_contain_entry(self, *args, **kwargs):
        ...
        for e in entries:
            ...
            if k in self.val and e[k] == self.val[k]:
                found.append(e)
        if found:
            self._err('Expected <%s> to not contain entries %s, but did contain %s.' % (
    ...
    def is_before(self, other):
        ...
        if self.val >= other:
            self._err('Expected <%s> to be before <%s>, but was not.' % (
                self.val.strftime('%Y-%m-%d %H:%M:%S'), other.strftime('%Y-%m-%d %H:%M:%S')))
        return self
    ...
    def is_after(self, other):
        ...
        if type(other) is not datetime.datetime:
            raise TypeError('given arg must be datetime, but was type <%s>' % type(other).__name__)
        if self.val <= other:
            self._err('Expected <%s> to be after <%s>, but was not.' % (
                self.val.strftime('%Y-%m-%d %H:%M:%S'), other.strftime('%Y-%m-%d %H:%M:%S')))
        return self
    ...
    def is_equal_to_ignoring_milliseconds(self, other):
        ...
        if self.val.date() != other.date() or self.val.hour != other.hour or self.val.minute != other.minute or self.val.second != other.second:
            self._err('Expected <%s> to be equal to <%s>, but was not.' % (
                self.val.strftime('%Y-%m-%d %H:%M:%S'), other.strftime('%Y-%m-%d %H:%M:%S')))
        return self
    ...
    def __getattr__(self, attr):
        ...
        def _wrapper(*args, **kwargs):
            if err_msg:
                ...
                expected = args[0]
                if actual != expected:
                    self._err('Expected <%s> to be equal to <%s> on %s <%s>, but was not.' % (
                ...
                for k in k1:
                    if self._check_dict_like(val[k], check_values=False, return_as_bool=True) and self._check_dict_like(
                        ...
                        return self._dict_not_equal(val[k], other[k],
                                                    ignore=[i[1:] for i in ignores if
                                                            type(i) is tuple and i[0] == k] if ignore else None,
                                                    include=[i[1:] for i in self._dict_ignore(include) if
                                                             type(i) is tuple and i[0] == k] if include else None)
                    elif val[k] != other[k]:
                        return True
            return False
        else:
            ...
            for k, v in d.items():
                if k not in other:
                    ...
                    out += '%s%s: %s' % (', ' if len(out) > 0 else '', repr(k),
                                         _dict_repr(v, other[k]) if self._check_dict_like(v, check_values=False,
                                                                                          return_as_bool=True) and self._check_dict_like(
