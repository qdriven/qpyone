...
def contents_of(f, encoding="utf-8"):
    ...
    try:
        ...
        try:
            ...
            raise ValueError(
                "val must be file or path, but was type <%s>" % type(f).__name__
            )
        except OSError:
...
class AssertionBuilder:
    ...
    def is_equal_to(self, other, **kwargs):
        ...
        if self._check_dict_like(
            self.val, check_values=False, return_as_bool=True
        ) and self._check_dict_like(other, check_values=False, return_as_bool=True):
    ...
    def is_not_equal_to(self, other):
        ...
        if self.val == other:
            self._err(
                "Expected <{}> to be not equal to <{}>, but was.".format(
                    self.val, other
                )
            )
        return self
    ...
    def is_type_of(self, some_type):
        ...
        if type(self.val) is not some_type:
            if hasattr(self.val, "__name__"):
                ...
                t = "unknown"
            self._err(
                "Expected <%s:%s> to be of type <%s>, but was not."
                % (self.val, t, some_type.__name__)
            )
        return self
        ...
        try:
            if not isinstance(self.val, some_class):
                if hasattr(self.val, "__name__"):
                    ...
                    t = "unknown"
                self._err(
                    "Expected <%s:%s> to be instance of class <%s>, but was not."
                    % (self.val, t, some_class.__name__)
                )
        except TypeError:
    ...
    def is_length(self, length):
        ...
        if len(self.val) != length:
            self._err(
                "Expected <%s> to be of length <%d>, but was <%d>."
        ...
        if len(items) == 0:
            ...
            if items[0] not in self.val:
                if self._check_dict_like(self.val, return_as_bool=True):
                    self._err(
                        "Expected <%s> to contain key <%s>, but did not."
                        % (self.val, items[0])
                    )
                else:
                    self._err(
                        "Expected <%s> to contain item <%s>, but did not."
                        % (self.val, items[0])
                    )
        else:
            missing = []
    ...
    def contains_sequence(self, *items):
        ...
        self._err(
            "Expected <%s> to contain sequence %s, but did not."
            % (self.val, self._fmt_items(items))
        )
        ...
        if len(items) == 0:
            ...
            for i in items:
                if self.val == i:
                    return self
        self._err(
            "Expected <%s> to be in %s, but was not."
            % (self.val, self._fmt_items(items))
        )
    ...
    def is_not_in(self, *items):
        ...
        if len(items) == 0:
            ...
            for i in items:
                if self.val == i:
                    self._err(
                        "Expected <%s> to not be in %s, but was."
                        % (self.val, self._fmt_items(items))
                    )
        return self
    ...
    def _validate_compareable(self, other):
        ...
        if self_type in self.NON_COMPAREABLE_TYPES:
            raise TypeError(
                "ordering is not defined for type <%s>" % self_type.__name__
            )
        if self_type in self.COMPAREABLE_TYPES:
            if other_type is not self_type:
                raise TypeError(
                    "given arg must be <%s>, but was <%s>"
                    % (self_type.__name__, other_type.__name__)
                )
            return
        if isinstance(self.val, numbers.Number):
            if not isinstance(other, numbers.Number):
                raise TypeError(
                    "given arg must be a number, but was <%s>" % other_type.__name__
                )
            return
    ...
    def is_close_to(self, other, tolerance):
        ...
        if self.val < (other - tolerance) or self.val > (other + tolerance):
            if type(self.val) is datetime.datetime:
                tolerance_seconds = (
                    tolerance.days * 86400
                    + tolerance.seconds
                    + tolerance.microseconds / 1000000
                )
    ...
    def is_equal_to_ignoring_case(self, other):
        ...
        if self.val.lower() != other.lower():
            self._err(
                "Expected <%s> to be case-insensitive equal to <%s>, but was not."
                % (self.val, other)
            )
        return self
        ...
        if isinstance(self.val, str_types):
            if len(items) == 1:
                ...
                if items[0].lower() not in self.val.lower():
                    self._err(
                        "Expected <%s> to case-insensitive contain item <%s>, but did not."
                        % (self.val, items[0])
                    )
            else:
                missing = []
    ...
    def starts_with(self, prefix):
        ...
        if isinstance(self.val, str_types):
            ...
            if not self.val.startswith(prefix):
                self._err(
                    "Expected <%s> to start with <%s>, but did not."
                    % (self.val, prefix)
                )
        elif isinstance(self.val, Iterable):
            if len(self.val) == 0:
            ...
            if first != prefix:
                self._err(
                    "Expected {} to start with <{}>, but did not.".format(
                        self.val, prefix
                    )
                )
        else:
    ...
    def ends_with(self, suffix):
        ...
        if isinstance(self.val, str_types):
            ...
            if not self.val.endswith(suffix):
                self._err(
                    "Expected <{}> to end with <{}>, but did not.".format(
                        self.val, suffix
                    )
                )
        elif isinstance(self.val, Iterable):
    ...
    def matches(self, pattern):
        ...
        if re.search(pattern, self.val) is None:
            self._err(
                "Expected <%s> to match pattern <%s>, but did not."
                % (self.val, pattern)
            )
        return self
    ...
    def does_not_match(self, pattern):
        ...
        if re.search(pattern, self.val) is not None:
            self._err(
                "Expected <%s> to not match pattern <%s>, but did."
                % (self.val, pattern)
            )
        return self
    ...
    def is_alpha(self):
        ...
        if not self.val.isalpha():
            self._err(
                "Expected <%s> to contain only alphabetic chars, but did not."
                % self.val
            )
        return self
    ...
    def is_lower(self):
        ...
        if self.val != self.val.lower():
            self._err(
                "Expected <%s> to contain only lowercase chars, but did not." % self.val
            )
        return self
    ...
    def is_upper(self):
        ...
        if self.val != self.val.upper():
            self._err(
                "Expected <%s> to contain only uppercase chars, but did not." % self.val
            )
        return self
    ...
    def is_unicode(self):
        ...
        if type(self.val) is not unicode:
            self._err(
                ...
                % (self.val, type(self.val).__name__)
            )
        return self
    ...
    def is_subset_of(self, *supersets):
        ...
        missing = []
        if (
            hasattr(self.val, "keys")
            and callable(getattr(self.val, "keys"))
    ...
    def is_before(self, other):
        ...
        if type(self.val) is not datetime.datetime:
            raise TypeError(
                "val must be datetime, but was type <%s>" % type(self.val).__name__
            )
        if type(other) is not datetime.datetime:
            raise TypeError(
                "given arg must be datetime, but was type <%s>" % type(other).__name__
            )
        if self.val >= other:
            self._err(
    ...
    def is_equal_to_ignoring_seconds(self, other):
        ...
        if (
            ...
            self._err(
                "Expected <%s> to be equal to <%s>, but was not."
                % (
                    self.val.strftime("%Y-%m-%d %H:%M"),
                    other.strftime("%Y-%m-%d %H:%M"),
                )
            )
        return self
    ...
    def extracting(self, *names, **kwargs):
        ...
        def _extract(x, name):
            if self._check_dict_like(x, check_values=False, return_as_bool=True):
                if name in x:
                    ...
                    raise ValueError(
                        "item keys {} did not contain key <{}>".format(
                            list(x.keys()), name
                        )
                    )
                ...
                raise ValueError(
                    "val does not have property or zero-arg method <%s>" % name
                )
        ...
        def _filter(x):
            if "filter" in kwargs:
                ...
                elif self._check_dict_like(
                    kwargs["filter"], check_values=False, return_as_bool=True
                ):
    ...
    def __getattr__(self, attr):
        ...
        if not hasattr(self.val, attr_name):
            if is_dict:
                if attr_name not in self.val:
                    err_msg = "Expected key <{}>, but val has no key <{}>.".format(
                        attr_name,
                        attr_name,
                    )
            else:
                err_msg = (
                    "Expected attribute <{}>, but val has no attribute <{}>.".format(
                        attr_name,
                        attr_name,
                    )
                )
        ...
        def _wrapper(*args, **kwargs):
            if err_msg:
                self._err(
                    err_msg
                )  # ok to raise AssertionError now that we are inside wrapper
            else:
                if len(args) != 1:
                    raise TypeError(
                        "assertion <%s()> takes exactly 1 argument (%d given)"
                        % (attr, len(args))
                    )
                ...
                if callable(val_attr):
                    try:
                        ...
                        raise TypeError(
                            "val does not have zero-arg method <%s()>" % attr_name
                        )
                else:
                    actual = val_attr
    ...
    def _err(self, msg):
        ...
        out = "{}{}".format(
            "[%s] " % self.description if len(self.description) > 0 else "",
            msg,
        )
        if self.kind == "warn":
            print(out)
    ...
    def _validate_between_args(self, val_type, low, high):
        ...
        if val_type in self.COMPAREABLE_TYPES:
            if low_type is not val_type:
                raise TypeError(
                    "given low arg must be <%s>, but was <%s>"
                    % (val_type.__name__, low_type.__name__)
                )
            if high_type is not val_type:
                raise TypeError(
                    "given high arg must be <%s>, but was <%s>"
                    % (val_type.__name__, low_type.__name__)
                )
        elif isinstance(self.val, numbers.Number):
            if isinstance(low, numbers.Number) is False:
                raise TypeError(
                    "given low arg must be numeric, but was <%s>" % low_type.__name__
                )
            if isinstance(high, numbers.Number) is False:
                raise TypeError(
                    "given high arg must be numeric, but was <%s>" % high_type.__name__
                )
        else:
            raise TypeError("ordering is not defined for type <%s>" % val_type.__name__)
    ...
    def _validate_close_to_args(self, val, other, tolerance):
        ...
        if (
            isinstance(val, numbers.Number) is False
            and type(val) is not datetime.datetime
        ):
        ...
        if type(val) is datetime.datetime:
            if type(other) is not datetime.datetime:
                raise TypeError(
                    "given arg must be datetime, but was <%s>" % type(other).__name__
                )
            if type(tolerance) is not datetime.timedelta:
                raise TypeError(
                    "given tolerance arg must be timedelta, but was <%s>"
    ...
    def _check_dict_like(
        ...
        if not isinstance(d, Iterable):
            if return_as_bool:
                ...
                raise TypeError(
                    "{} <{}> is not dict-like: not iterable".format(
        ...
        if check_keys:
            if not hasattr(d, "keys") or not callable(getattr(d, "keys")):
                if return_as_bool:
                    ...
                    raise TypeError(
                        "%s <%s> is not dict-like: missing keys()"
                        % (name, type(d).__name__)
                    )
        if check_values:
            if not hasattr(d, "values") or not callable(getattr(d, "values")):
                if return_as_bool:
                    ...
                    raise TypeError(
                        "%s <%s> is not dict-like: missing values()"
                        % (name, type(d).__name__)
                    )
        if check_getitem:
            if not hasattr(d, "__getitem__"):
                if return_as_bool:
                    ...
                    raise TypeError(
                        "%s <%s> is not dict-like: missing [] accessor"
                        % (name, type(d).__name__)
                    )
        if return_as_bool:
            return True
    ...
    def _check_iterable(self, l, check_getitem=True, name="val"):
        if not isinstance(l, Iterable):
            raise TypeError(f"{name} <{type(l).__name__}> is not iterable")
        if check_getitem:
            if not hasattr(l, "__getitem__"):
                raise TypeError(
                    f"{name} <{type(l).__name__}> does not have [] accessor"
                )
    ...
    def _dict_not_equal(self, val, other, ignore=None, include=None):
        if ignore or include:
            ...
            if k1 != k2:
                ...
                for k in k1:
                    if self._check_dict_like(
                        val[k], check_values=False, return_as_bool=True
                    ) and self._check_dict_like(
                        ...
                        return self._dict_not_equal(
                            ...
                            other[k],
                            ignore=[
                                i[1:] for i in ignores if type(i) is tuple and i[0] == k
                            ]
                            if ignore
                            else None,
                            include=[
                                i[1:]
                                for i in self._dict_ignore(include)
                                if type(i) is tuple and i[0] == k
                            ]
                            if include
                            else None,
                        )
    ...
    def _dict_err(self, val, other, ignore=None, include=None):
        def _dict_repr(d, other):
            ...
            for k, v in d.items():
                if k not in other:
                    out += "{}{}: {}".format(
                        ", " if len(out) > 0 else "", repr(k), repr(v)
                    )
                elif v != other[k]:
                    out += "{}{}: {}".format(
                        ", " if len(out) > 0 else "",
                        ...
                        if self._check_dict_like(
                            v, check_values=False, return_as_bool=True
                        )
                        and self._check_dict_like(
                            other[k], check_values=False, return_as_bool=True
                        )
                    ...
                    ellip = True
            return "{{{}{}}}".format(
                ".." if ellip and len(out) == 0 else ".., " if ellip else "",
                out,
            )
        ...
        if ignore:
            ...
            ignore_err = " ignoring keys %s" % self._fmt_items(
                [
                    ".".join([str(s) for s in i]) if type(i) is tuple else i
                    for i in ignores
                ]
            )
        if include:
    ...
    def snapshot(self, id=None, path="__snapshots"):
        ...
        class _Encoder(json.JSONEncoder):
            def default(self, o):
                if isinstance(o, set):
                    ...
                    return {
                        "__type__": "datetime",
                        "__data__": o.strftime("%Y-%m-%d %H:%M:%S"),
                    }
        ...
        def _name(path, name):
            try:
                return os.path.join(
                    path, "snap-%s.json" % name.replace(" ", "_").lower()
                )
            except Exception:
                raise ValueError(
                    "failed to create snapshot filename, either bad path or bad name"
                )
