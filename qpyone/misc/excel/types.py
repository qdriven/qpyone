...
class Money(float, metaclass=ABCMeta):
    ...
    def number_format(cls) -> str:
        ...
        if cls.code_before_amount:
            amount = f"{cls.code} {decimal_seperation}"
        else:
            amount = f"{decimal_seperation} {cls.code}"
        rsl = f"{amount};[RED]-{amount}"
        return rsl
