...
class Composition(metaclass=ABCMeta):
    ...
    def __init__(self, data: list["XlsxModel"], title: str, config: FXlsxConfig):
        if len(data) < 1:
            raise ValueError(
                "cannot create the {} sheet, at least one entry \
...
True.".format(
    ...
    def __apply_apperance(self, ws: Worksheet) -> None:
        ...
        for col in range(1, ws.max_column + 1):
            ...
            if hasattr(field_info, "font") and field_info.font is not None:
                column.font = field_info.font
            # 2nd font priority: Derived from FieldTypeInfo.
            elif hasattr(field_type_info, "font") and field_type_info.font is not None:
                column.font = field_info.font
            ...
            if (
                hasattr(field_info, "number_format")
                and field_info.number_format is not None
            ):
                column.number_format = field_info.number_format
            # 2nd number format priority: Derived from FieldTypeInfo.
            elif (
                hasattr(field_type_info, "number_format")
                and field_type_info.number_format is not None
            ):
    ...
    def __apply_print_settings(self, ws: Worksheet) -> None:
        ...
        ws.print_options.horizontalCentered = self.config.print_horizontal_centered
        ws.print_options.verticalCentered = self.config.print_vertical_centered
    ...
    def __data_table(self, ws: Worksheet) -> None:
        ...
        table = Table(displayName=self.title.replace(" ", ""), ref=self.__dimensions())
        ws.add_table(table)

    def __dimensions(self) -> str:
        """Returns the dimension of the used space."""
        return "A1:{col}{row}".format(
            col=pyxl_utils.cell.get_column_letter(len(self.data[0].__fields__)),
            row=len(self.data) + 1,
        )
    ...
    def __validator_for_enum(enum: Enum) -> DataValidation:
        ...
        dv.error = f"Your entry is not a valid {name}"
        dv.errorTitle = f"Invalid {name}"
        dv.prompt = f"Select {name} from the list"
        dv.promptTitle = f"Select {name}"
...
class SingleModelComposition(Composition):
    ...
    @classmethod
    def sheets_from_model(cls, model: "XlsxModel") -> list["SingleModelComposition"]:
...
class RootCollectionComposition(Composition):
    ...
    @classmethod
    def sheets_from_model(cls, model: "XlsxModel") -> list["RootCollectionComposition"]:
        if (schema_title := model.__config__.title) is not None:
            title = schema_title
        else:
...
class CollectionComposition(Composition):
    ...
    @classmethod
    def sheets_from_model(cls, model: "XlsxModel") -> list["CollectionComposition"]:
        rsl: list["CollectionComposition"] = []
        for key, field in model.__fields__.items():
            rsl.append(cls(model.__dict__[key], field.alias, model.__config__))
        return rsl
...
class CompositionFactory:
    ...
    def from_model(
        ...
        RootCollectionComposition
        | CollectionComposition
        | SingleModelComposition
        | Composition
    ]:
        """
        ...
        for key, prop in model.__fields__.items():
            ...
            if typ is not None and issubclass(typ, list) and "XlsxModel" in bases:
