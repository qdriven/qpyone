...
class IdentityMixin(object):
    ...
    def auth_token(self):
        ...
        payload = {
            ...
            + datetime.timedelta(seconds=IdentityMixin.token_validity_in_seconds),
        ...
        return jwt.encode(payload, key=config.private_key, algorithm="RS256").decode(
            "utf-8"
        )
        ...
        def add_endpoint_and_permissions(meth):
            ...
            if meth not in cls.protected_methods:
                cls.protected_methods[meth] = {
                    endpoint or "*": permissions
                    if isinstance(permissions, list)
                    else [permissions]
                }
            else:
                cls.protected_methods[meth][endpoint or "*"] = (
                    permissions if isinstance(permissions, list) else [permissions]
                )

        if (
            not isinstance(permissions, list)
            and not isinstance(permissions, Permission)
        ) or (
            isinstance(permissions, list)
            and len([perm for perm in permissions if isinstance(perm, Permission)]) == 0
        ):
            raise AttributeError(
                "The permission must be a subclass of a Permission or list of Permissions"
            )
