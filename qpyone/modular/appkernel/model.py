...
format_map = {"datetime": "date-time", "time": "time"}
...
def _xtract_custom_object_to_dict(custom_object, converter_func=None):
    if hasattr(custom_object, "__dict__"):
        instance_items = set(
            [
                (pn, pv)
                for pn, pv in custom_object.__dict__.items()
    ...
    result = {}
    result.update(
        _type=f"{custom_object.__module__}.{custom_object.__class__.__qualname__}"
    )

    for prop_name, prop_value in instance_items:
        result[prop_name] = _xtract_custom_object_to_dict(prop_value)
    properties = set(
        inspect.getmembers(custom_object.__class__, lambda o: isinstance(o, property))
    )
...
OPS = AttrDict(  # pylint: disable=C0103
    ...
    ELEM_DOES_NOT_MATCH=Opex(
        "$elemMatchNot", lambda exp: {"$not": {"$elemMatch": {exp[0]: exp[1]}}}
    ),
    ELEM_LIKE=Opex(
        "$elemMatch",
        lambda exp: {
            "$elemMatch": {exp[0]: {"$regex": ".*{}.*".format(exp[1]), "$options": "i"}}
        },
    ),
...
class CustomProperty(DslBase):
    def __init__(self, cls, property_name):
        self.backreference = BackReference(
            class_name=cls.__name__, parameter_name=property_name
        )
...
def get_argument_spec(provisioner_method):
    ...
    assert inspect.ismethod(provisioner_method) or inspect.isfunction(
        provisioner_method
    ), f"The provisioner {str(provisioner_method)} method must be a method or function"
    args = [
        name
        for name in getattr(inspect.getfullargspec(provisioner_method), "args")
...
def tag_class_items(class_name, class_dictionary):
    ...
    for member_name, member in class_dictionary.items():
        if hasattr(member, "member_tag") and (
        ...
        if isinstance(member, Property):
            ...
            member.backreference = BackReference(
                class_name=class_name, parameter_name=member_name
            )
    return tags
...
class Property(DslBase):
    ...
    def __getattr__(self, attribute):
        if self.python_type == list and (
            ...
            and self.sub_type
            and issubclass(self.sub_type, Model)
        ):
            if hasattr(self.sub_type, attribute):
                nested_parameter = getattr(self.sub_type, attribute)
                nested_parameter.backreference.array_parameter_name = (
                    self.backreference.parameter_name
                )
            ...
            if hasattr(self.python_type, attribute):
                ...
                nested_parameter.backreference.parameter_name = "{}.{}".format(
                    self.backreference.parameter_name, attribute
                )
                return nested_parameter
        raise AttributeError(
            "Class {} has no attribute {}".format(self.__class__.__name__, attribute)
        )
    ...
    def __getitem__(self, item_expression):
        ...
        if (
            ...
            and issubclass(self.sub_type, Model)
            and self.sub_type.__name__ == item_expression.lhs.backreference.class_name
        ):
            item_expression.lhs.backreference.within_an_array = True
            item_expression.lhs.backreference.array_parameter_name = (
                self.backreference.parameter_name
            )
            ...
            raise TypeError(
                "The subtype {} of the parameter is not {}".format(
                    self.sub_type, item_expression.lhs.backreference.class_name
                )
            )  # if the type of the key is wrong
...
class Model(object, metaclass=_TaggingMetaClass):
        ...
        for name in kwargs:
            if name in self.__dict__:
                ...
                if isinstance(attr, list):
                    ...
                    raise AttributeError(
                        "The attribute {} is not a list on {}.".format(
                            name, self.__class__.__name__
                        )
                    )
            else:
                raise AttributeError(
                    "The attribute {} is missing from the {} class.".format(
                        name, self.__class__.__name__
                    )
                )
        return self
    ...
    def __str__(self):
        return "<{}> {}".format(
            ...
            Model.to_dict(self, validate=False, marshal_values=False),
        )
    ...
    def init_model(instance, **kwargs):
        if isinstance(instance, Model):
            ...
            raise TypeError(
                "The Model initialisation works only with instances which inherit from Model."
            )

    @classmethod
    def get_json_schema(
        cls, additional_properties: bool = True, mongo_compatibility: bool = False
    ) -> dict:
        """
        ...
        (
            ...
            required_props,
            definitions,
        ) = Model.__prepare_json_schema_properties(
    ...
    def __prepare_json_schema_properties(specs, mongo_compatibility=False):
        ...
        for name, spec in specs.items():
            ...
            type_string = (
                spec_type.__name__ if hasattr(spec_type, "__name__") else str(spec_type)
            )
            if mongo_compatibility and (name == "id" or name == "_id"):
                ...
                if not mongo_compatibility:
                    properties[name] = {
                        type_label: [type_map.get(type_string, "string")]
                    }
                    # -- define formats --
                    ...
                    properties[name] = {
                        type_label: [bson_type_map.get(type_string, "string")]
                    }
                if type_string == "int":
            ...
            if spec.get("type") == list and spec.get("sub_type"):
                ...
                subtype_string = (
                    subtype.__name__ if hasattr(subtype, "__name__") else str(subtype)
                )
                if not isinstance(subtype, dict):
                    # subtype is a primitive
                    if not mongo_compatibility:
                        properties[name].update(
                            items={type_label: type_map.get(subtype_string, "string")}
                        )
                    else:
                        properties[name].update(
                            items={
                                type_label: bson_type_map.get(subtype_string, "string")
                            }
                        )
                    ...
                    if not mongo_compatibility:
                        ...
                        properties[name].update(
                            items={
                                "oneOf": [{"$ref": "#/definitions/{}".format(def_name)}]
                            }
                        )
                    else:
    ...
    def get_parameter_spec(cls, convert_types_to_string=True):
        ...
        for field_name in props:
            ...
            if isinstance(attribute, Property):
                result_dct[field_name] = Model.__describe_attribute(
                    cls,
                    field_name,
                    attribute,
    ...
    def __describe_attribute(
        clazz, field_name, attribute, convert_types_to_string=True
    ):
        attr_desc = {
            "type": attribute.python_type.__name__
            if convert_types_to_string
            else attribute.python_type,
        ...
        if issubclass(attribute.python_type, Model):
            attr_desc.update(
                props=attribute.python_type.get_parameter_spec(
                    convert_types_to_string=convert_types_to_string
                )
            )
        ...
        if attribute.sub_type:
            if issubclass(attribute.sub_type, Model):
                attr_desc.update(
                    sub_type={
                        "type": attribute.sub_type.__name__
                        if convert_types_to_string
                        else attribute.sub_type,
                        "props": attribute.sub_type.get_parameter_spec(
                            convert_types_to_string=convert_types_to_string
                        ),
                    }
                )
                ...
                attr_desc.update(
                    sub_type=attribute.sub_type.__name__
                    if convert_types_to_string
                    else attribute.sub_type
                )
        if attribute.validators:
            if not isinstance(attribute.validators, list) and (
                (
                    inspect.isclass(attribute.validators)
                    and issubclass(attribute.validators, Validator)
                )
            ...
            attr_desc.update(
                validators=[
                    clazz.__describe_validator(
                        val, convert_types_to_string=convert_types_to_string
                    )
                    for val in attribute.validators
                ]
            )
        return attr_desc
    ...
    def __describe_validator(validator, convert_types_to_string=True):
        ...
        val_desc = {
            "type": get_value(validator)
            if hasattr(validator, "__name__")
            else get_value(validator.__class__)
        }
    ...
    def to_dict(
        ...
        result = {}
        instance_items = (
            list(instance.__dict__.items())
            if not isinstance(instance, dict)
            else list(instance.items())
        )
        cls_items = {
            k: v
        ...
        for param, obj in instance_items:
            ...
            if isinstance(obj, Model):
                result[param] = Model.to_dict(
                    obj, convert_id, converter_func=converter_func
                )
            elif isinstance(obj, Enum):
                result[param] = obj.name
            elif isinstance(obj, list):
                result[param] = [
                    Model.to_dict(list_item, convert_id, converter_func=converter_func)
                ...
                if (
                    class_property
                    and isinstance(class_property, Property)
                    and class_property.marshaller
                ):
                    if isinstance(class_property.marshaller, type) and issubclass(
                        class_property.marshaller, Marshaller
                    ):
                        class_property.marshaller = class_property.marshaller()
                    if (
                        isinstance(class_property.marshaller, Marshaller)
                        and marshal_values
                    ):
                ...
                if convert_id and param == "id":
                    ...
                    result_value = _xtract_custom_object_to_dict(
                        result_value, converter_func=converter_func
                    )
                    result[param] = result_value
        if hasattr(instance, "__module__"):
            result.update(
                _type=f"{instance.__module__}.{instance.__class__.__qualname__}"
            )
        else:
    ...
    def from_dict(
        ...
        instance = cls()
        class_variables = [
            f for f in set(dir(instance)) if Model.__is_param_field(f, cls)
        ]
        ...
        if dict_obj and isinstance(dict_obj, dict):
                ...
                if key in class_variables:
                    ...
                    if isinstance(parameter, Property):
                        if parameter.marshaller:
                            if isinstance(parameter.marshaller, type) and issubclass(
                                parameter.marshaller, Marshaller
                            ):
                        ...
                        if issubclass(parameter.python_type, Model):
                            ...
                            setattr(
                                instance,
                                key,
                                string_to_type_converters.get(
                                    parameter.python_type, default_convert
                                )(val),
                            )
                        else:
                            ...
                            setattr(
                                instance,
                                key,
                                Model.load_and_or_convert_object(
                                    val, converter_func=converter_func
                                ),
                            )
                elif (
                    (key == "_id" or key == "id")
                    and isinstance(val, (str, bytes))
                    and val.startswith(OBJ_PREFIX)
                ):
            ...
            for nullable in (
                set(class_variables).union(unmanaged_parameters) - processed_properties
            ):
    ...
    def load_and_or_convert_object(custom_value, converter_func=None):
        if custom_value and isinstance(custom_value, dict) and "_type" in custom_value:
            ...
            custom_value = _instantiate_custom_class(
                custom_class, custom_value, converter_func=converter_func
            )
        if converter_func and isinstance(converter_func, Callable):
    ...
    def dumps(
        ...
        json_serialiser_func: Callable = None,
    ) -> str:
        """
        ...
        default_serialiser_func = (
            json_serialiser_func
            if json_serialiser_func and isinstance(json_serialiser_func, Callable)
            else default_json_serializer
        )
        return json.dumps(
            model_as_dict,
            default=default_serialiser_func,
    ...
    def finalise_and_validate(self):
        ...
        for param_name, param_object in list(cls_items.items()):
            ...
            # validate fields
            if param_object.required and (
                param_name not in obj_items or obj_items.get(param_name) is None
            ):
            ...
            if param_object.converter and param_name in self.__dict__:
                setattr(
                    self, param_name, param_object.converter(getattr(self, param_name))
                )
            if param_object.validators and isinstance(
                param_object.validators, (list, set)
            ):
            ...
            if (
                issubclass(param_object.python_type, (list, set))
                and param_name in obj_items
            ):
    ...
    def __check_validity(validator: Validator, param_name, obj_items):
        if isinstance(validator, Validator) and param_name in obj_items:
            ...
            isinstance(validator, type)
            and issubclass(validator, Validator)
            and param_name in obj_items
        ):
            validator().validate_objects(param_name, obj_items)
    ...
    def dump_spec(self):
        ...
        print(
            "params: %s"
            % [f for f in props if self.__is_param_field(f, self.__class__)]
        )
    ...
    def __include_instance(self, field):
        return (
            ...
            and not isinstance(getattr(self, field), collections.Callable)
            and not isinstance(getattr(self, field), Property)
        )
