...
class ValidationException(AppKernelException):
    def __init__(self, message, validable_object=None, validator_type=None):
        self.validable_object_name = (
            validable_object.__class__.__name__ if validable_object else "unspec"
        )
        validator_name = str(validator_type) if validator_type else "unspec"
        super().__init__(
            f"{validator_name} on type {self.validable_object_name} - {message}"
        )
    ...
    def validate(self, parameter_name, validable_object):
        if isinstance(validable_object, str):
            if not re.match(self.value, validable_object):
                raise ValidationException(
                    _(
                        "The property %(prop_name)s cannot be validated against %(value)s.",
                        prop_name=parameter_name,
                        value=self.value,
                    ),
                    self.type,
                    validable_object,
...
class Min(Validator):
    ...
    def validate(self, parameter_name, validable_object):
        if isinstance(validable_object, (int, float)) and validable_object < self.value:
            raise ValidationException(
                f"The parameter {parameter_name} should hava a min. value of {self.value}",
                validable_object,
                self.type,
            )
...
class Max(Validator):
    ...
    def validate(self, parameter_name, validable_object):
        if isinstance(validable_object, (int, float)) and validable_object > self.value:
            raise ValidationException(
                f"The parameter {parameter_name} should have a max. value of {self.value}",
                validable_object,
                self.type,
            )
    ...
    def validate(self, parameter_name, validable_object):
        if validable_object and isinstance(validable_object, list):
            if len(set(validable_object)) != len(validable_object):
                raise ValidationException(
                    f"The parameter {parameter_name} must not contain duplicated elements",
                    validable_object,
                    self.type,
                )
...
class NotEmpty(Validator):
    ...
    def validate(self, parameter_name, validable_object):
        if (
            not validable_object
            or not isinstance(validable_object, (str, list, set, dict, tuple))
            or len(validable_object) == 0
        ):
            raise ValidationException(
                f"The parameter {parameter_name} is None or empty.",
                validable_object,
                self.type,
            )
...
class Past(Validator):
    ...
    def validate(self, parameter_name, validable_object):
        if validable_object is None:
            ...
            raise ValidationException(
                f"The parameter {parameter_name} is none or not date type.",
                validable_object,
                self.type,
            )
        elif validable_object >= datetime.datetime.now():
            raise ValidationException(
                f"The parameter {parameter_name} must define the past.",
                validable_object,
                self.type,
            )
...
class Future(Validator):
    ...
    def validate(self, parameter_name, validable_object):
        if validable_object is None or not self._is_date(validable_object):
            ...
            raise ValidationException(
                f"The parameter {parameter_name} must define the future.",
                validable_object,
                self.type,
            )
