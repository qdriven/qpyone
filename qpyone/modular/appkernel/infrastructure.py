...
class CfgEngine(object):
    ...
    def __init__(self, cfg_dir, config_file_name="cfg.yml", optional=False):
        ...
        with open(config_file, "r") as ymlfile:
            try:
                ...
                raise AppInitialisationError(
                    f"cannot read config file {config_file} due to: {str(se)}"
                )
    ...
    def get_value_for_path_list(
        self, config_nodes, section_dict=None, default_value=None
    ):
        """
        ...
        if len(config_nodes) == 0:
            ...
            return self.get_value_for_path_list(
                config_nodes, section_dict.get(key), default_value=default_value
            )
        else:
            return default_value
