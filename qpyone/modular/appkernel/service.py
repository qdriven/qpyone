...
from .model import tag_class_items
from .query import QueryProcessor
from .reflection import is_dictionary
from .reflection import is_dictionary_subclass
from .reflection import is_noncomplex
from .reflection import is_primitive
...
def _add_app_rule(
    ...
    **options,
    ...
    if path_param and path_param.startswith("./"):
        ...
        rule = f"{base_name}/"
    endpoint = "{}_{}_{}".format(
        clazz_name, method_name, options.get("methods")[0].lower()
    )
...
model_endpoints = {
    "find_by_query": [
        {
            "func": lambda cls, engine: _execute(
                cls, engine, _hook(cls, cls.find_by_query, "on_get"), cls
            ),
            "method": "GET",
        }
    ],
    "find_by_id": [
        {
            "func": lambda cls, engine: _execute(
                cls, engine, _hook(cls, cls.find_by_id, "get"), cls
            ),
            "method": "GET",
    ...
    "save_object": [
        {
            "func": lambda cls, engine: _execute(
                cls, engine, _hook(cls, cls.save_object, "post"), cls
            ),
            "method": "POST",
        },
        {
            "func": lambda cls, engine: _execute(
                cls, engine, _hook(cls, cls.patch_object, "patch"), cls
            ),
            "method": "PATCH",
            "param": "<string:object_id>",
        },
    ...
    "replace_object": [
        {
            "func": lambda cls, engine: _execute(
                cls, engine, _hook(cls, cls.replace_object, "put"), cls
            ),
            "method": "PUT",
        }
    ],
    "delete_by_id": [
        {
            "func": lambda cls, engine: _execute(
                cls, engine, _hook(cls, cls.delete_by_id, "delete"), cls
            ),
            "method": "DELETE",
            "param": "<object_id>",
        }
...
def expose_service(
    ...
    clazz = (
        ...
        else clazz_or_instance.__class__
    )
    ...
    class_methods = [
        cm
        for cm in dir(clazz_or_instance)
        if not cm.startswith("_") and callable(getattr(clazz_or_instance, cm))
    ]
    if inspect.isclass(clazz_or_instance):
        if issubclass(clazz_or_instance, Model):
            _add_app_rule(
                ...
                url_base,
                "schema",
                _create_simple_wrapper_executor(
                    clazz_or_instance, app_engine, clazz_or_instance.get_json_schema
                ),
                path_param="schema",
                methods=["GET"],
            )
            _add_app_rule(
                ...
                "meta",
                _create_simple_wrapper_executor(
                    clazz_or_instance, app_engine, clazz_or_instance.get_parameter_spec
                ),
                path_param="meta",
                methods=["GET"],
            )
                ...
                if mdef_list:
                    for mdef in mdef_list:
                        ...
                        _add_app_rule(
                            ...
                            url_base,
                            method,
                            func(clazz_or_instance, app_engine),
    ...
    cls_items = (
        clazz_or_instance.__dict__
        if inspect.isclass(clazz_or_instance)
        else clazz_or_instance.__class__.__dict__
    )
    _prepare_actions(
        clazz_or_instance
        if inspect.isclass(clazz_or_instance)
        ...
        url_base,
        enable_security=setup_security,
        class_items=cls_items,
    )
    _prepare_resources(
        clazz_or_instance,
        url_base,
...
def __get_http_methods(tagged_item):
    ...
    http_methods = tagged_item.get("decorator_kwargs").get(
        "method", default_method(tag_args)
    )
...
def _prepare_resources(
    clazz_or_instance, url_base: str, enable_security: bool = False, class_items=None
):
            ...
            try:
                ...
                if "_type" in payload:
                    ...
                    result = executable_method(
                        mdl,
                        **_autobox_parameters(
                            executable_method, request_and_posted_arguments
                        ),
                    )
                else:
                ...
                return config.app_engine.generic_error_handler(
                    exc, upstream_service=clazz.__name__
                )
    ...
    if "resources" in class_items:
        for resource in class_items.get("resources"):
            ...
            path_segment = resource.get("decorator_kwargs").get(
                "path", f"./{func_name.lower()}"
            )
            _add_app_rule(
                clazz_or_instance,
                url_base,
                func_name,
                create_resource_executor(func_name),
                path_param=path_segment,
                methods=methods,
            )

        if enable_security:
            required_permissions = resource.get("decorator_kwargs").get(
                "require", Denied()
            )
            RbacMixin.set_list(
                cls=clazz_or_instance,
                methods=methods,
                permissions=required_permissions,
                endpoint="{}_{}_{}".format(
                    xtract(clazz_or_instance).lower(), func_name, methods[0].lower()
                ),
            )


def _prepare_actions(
    cls, url_base: str, enable_security: bool = False, class_items=None
):
        ...
        def action_executor(**named_args):
            if "object_id" not in named_args:
                ...
                try:
                    ...
                    config.app_engine.logger.warning(
                        "Service error: {}".format(str(sexc))
                    )
                    return create_custom_error(
                        sexc.http_error_code, sexc.message, cls.__name__
                    )
                except Exception as exc:
                    config.app_engine.logger.exception(exc)
                    return config.app_engine.generic_error_handler(
                        exc, upstream_service=cls.__name__
                    )
    ...
    if "actions" in class_items:
        for this_link in cls.actions:
            ...
            _add_app_rule(
                ...
                path_param="<object_id>/{}".format(relation),
                methods=methods,
            )
            if enable_security:
                required_permissions = this_link.get("decorator_kwargs").get(
                    "require", Denied()
                )
                RbacMixin.set_list(
                    cls=cls,
                    ...
                    permissions=required_permissions,
                    endpoint="{}_{}_{}".format(
                        xtract(cls).lower(), relation, methods[0].lower()
                    ),
                )
...
def _xtract_form():
    ...
    return dict(
        (key, value[0] if len(value) == 1 else value) for key, value in target.items()
    )
...
def _create_simple_wrapper_executor(cls, app_engine, provisioner_method):
    def create_executor(*args, **named_args):
        try:
            ...
            return app_engine.generic_error_handler(
                genex, upstream_service=cls.__name__
            )
...
def _execute(
    cls, app_engine: AppKernelEngine, provisioner_method: Callable, model_class: Model
):
    """
    ...
    """
    executable_method = (
        provisioner_method.inner_function
        if isinstance(provisioner_method, types.FunctionType)
        and hasattr(provisioner_method, "inner_function")
        else provisioner_method
    )
    ...
    def create_executor(**named_args):
        try:
            ...
            if request.method in ["POST", "PUT"]:
                ...
                model_instance = Model.from_dict(
                    _extract_dict_from_payload(), model_class
                )
                ...
                named_and_request_arguments.update(
                    document=_extract_dict_from_payload()
                )
            ...
            if request.method == "DELETE" and isinstance(result, int) and result == 0:
                return create_custom_error(
                    404,
                    "Document with id {} was not deleted.".format(
            ...
            app_engine.logger.warning(
                "missing parameter: {}/{}".format(pexc.__class__.__name__, str(pexc))
            )
            ...
            return create_custom_error(
                400, "{}/{}".format(vexc.__class__.__name__, str(vexc)), cls.__name__
            )
            ...
            return create_custom_error(
                rexc.status_code, rexc.message, rexc.upstream_service
            )
        except Exception as exc:
    ...
    for query_item in [[key, val] for key, val in query_dict.items()]:
        ...
        if isinstance(query_item[1], list) and len(query_item[1]) > 1:
            ...
            if isinstance(value_list[0], tuple):
                expression_list.append({query_item[0]: dict(value_list)})
            else:
            ...
            if date_pattern.match(expression):
                for parser_format_matcher in qp.date_separator_patterns.keys():
                    if parser_format_matcher.match(expression):
                        ...
                        separator = qp.date_separator_patterns.get(
                            parser_format_matcher
                        )
                        return datetime.strptime(
                            expression, date_parser_pattern.format(separator)
                        )
    return expression
...
def _autobox_parameters(provisioner_method, arguments):
    ...
    for arg_key, arg_value in arguments.items():
        ...
        if (
            required_type is not type(None)
            and provided_type is not type(None)
            and required_type != provided_type
        ):
            if issubclass(required_type, Enum):
                ...
                if qp.csv_pattern.match(arg_value):
                    arguments[arg_key] = [
                        int(item) if item.isdigit() else item.strip('"').strip("'")
...
def _xvert(cls, result_item, generate_links=True):
    ...
    if isinstance(result_item, Model):
        ...
        if hasattr(cls, "enable_hateoas") and cls.enable_hateoas:
            result.update(
                _links={
                    "self": {
                        "href": url_for(
                            "{}_find_by_query_get".format(xtract(cls).lower())
                        )
                    }
                }
            )
        ...
        is_primitive(result_item)
        or isinstance(result_item, (str, int))
        or is_noncomplex(result_item)
    ):
...
def _calculate_links(cls, object_id):
    ...
    if "actions" in all_members:
        for this_link in all_members.get("actions"):
            ...
            http_methods = decorator_args.get(
                "method", "POST" if len(args) > 0 else "GET"
            )
            endpoint_name = "{}_{}_{}".format(
                clazz_name,
                rel,
                http_methods[0].lower()
                if isinstance(http_methods, list)
                else http_methods.lower(),
            )
            href = "{}".format(url_for(endpoint_name, object_id=object_id))

            links[rel] = {"href": href, "methods": http_methods}
            if args and len(args) > 0:
                links[rel].update(args=args)

        links["self"] = {
            "href": url_for(
                "{}_find_by_id_get".format(clazz_name), object_id=object_id
            ),
            "methods": cls.methods,
        }
        links["collection"] = {
