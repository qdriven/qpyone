...
from appkernel import AppKernelException
...
class RequestWrapper(object):
    ...
    def get_headers():
        ...
        headers["Accept-Language"] = (
            accept_lang.best if not isinstance(accept_lang, str) else accept_lang
        )
        return headers
    ...
    def __execute(self, func, **kwargs):
        try:
            ...
            if path_ext:
                endpoint_url = "{}/{}".format(
                    self.url.rstrip("/"), path_ext.lstrip("/")
                )
            else:
            ...
            if 200 <= response.status_code <= 299:
                ...
                if "_type" in response_object and response_object.get("_type") not in [
                    "OperationResult",
                    "ErrorMessage",
                ]:
                    type_class = _get_custom_class(response_object.pop("_type"))
                    return response.status_code, Model.from_dict(
                        response_object, type_class
                    )
                else:
            ...
            if (
                "_type" in content
                and content.get("_type") == MessageType.ErrorMessage.name
            ):
                msg = content.get("message")
                upstream = content.get(
                    "upstream_service", self.url.rstrip("/").split("/").pop()
                )
                ...
                raise RequestHandlingException(
                    response.status_code, "Error while calling service."
                )
...
class HttpClientServiceProxy(object):
    ...
    def wrap(self, resource_path: str):
        return RequestWrapper(
            f'{self.root_url}/{resource_path.lstrip("/")}', session=self.session
        )
