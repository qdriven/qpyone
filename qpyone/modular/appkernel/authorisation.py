...
def __has_current_subject_authority(token: dict, authority):
    if not isinstance(authority, iam.CurrentSubject):
        raise TypeError(
            "This method required authority of type {}.".format(
                iam.CurrentSubject.__name__
            )
        )
...
def __contains(required_permissions: list, permission_type: Type):
    contained_permissions = [
        ...
        if isinstance(permission, permission_type)
    ]
...
def __has_required_authority(required_authorities: list, token: dict):
    if not required_authorities or len(required_authorities) == 0:
        ...
        for required_authority in required_authorities:
            check_authority = authority_evaluators.get(
                required_authority.__class__.__name__
            )
...
authority_evaluators = {iam.CurrentSubject.__name__: __has_current_subject_authority}
...
def authorize_request():
    ...
    try:
        ...
        required_roles, required_authorities = __split_to_roles_and_authorities(
            required_permissions
        )

        missing_required_role = (
            len(required_roles.intersection(set(token.get("roles", [])))) == 0
        )
        missing_required_authority = not __has_required_authority(
            required_authorities, token
        )
