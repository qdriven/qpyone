...
def get_cmdline_options():
    ...
    opts, args = getopt.getopt(
        argv, "c:dw:h:", ["config-dir=", "development", "working-dir=", "db-host="]
    )
    ...
    if config_dir_param:
        ...
        if not os.path.isdir(cfg_dir) or not os.access(cfg_dir, os.W_OK):
            raise AppInitialisationError(
                "The config directory [{}] is not found/not writable.".format(cfg_dir)
            )
    else:
        cfg_dir = None
    ...
    if working_dir_param:
        ...
        if not os.path.isdir(cwd) or not os.access(cwd, os.W_OK):
            raise AppInitialisationError(
                "The working directory[{}] is not found/not writable.".format(cwd)
            )
    else:
        cwd = f'{cwd.rstrip("/")}'
    development = get_option_value(("-d", "--development"), opts)
    db_host = get_option_value(("-h", "--db-host"), opts)
    return {"cfg_dir": cfg_dir, "development": development, "cwd": cwd, "db": db_host}
...
class AppKernelEngine(object):
    ...
    def __init_event_loop():
        ...
        def shutdown_eventloop():
            if (
                "event_loop" in config.__dict__
                and config.event_loop
                and config.event_loop.is_running()
            ):
    ...
    def __init_babel(self):
        ...
        supported_languages = []
        for supported_lang in self.cfg_engine.get(
            "appkernel.i18n.languages", ["en-US"]
        ):
    ...
    def get_cmdline_options():
        ...
        if config_dir_provided:
            ...
            if not os.path.isdir(cfg_dir) or not os.access(cfg_dir, os.W_OK):
                raise AppInitialisationError(
                    "The config directory [{}] is not found/not writable.".format(
                        cfg_dir
                    )
                )
        else:
    ...
    def init_logger(self, log_folder, level=logging.DEBUG):
        ...
        if self.development:
            ...
            file_name = (
                self.cfg_engine.get("appkernel.logging.file_name")
                or f"{self.app_id.replace(' ', '_').lower()}.log"
            )
            handler = RotatingFileHandler(
                "{}/{}".format(log_folder, file_name),
                maxBytes=max_bytes,
    ...
    def generic_error_handler(self, ex: Exception = None, upstream_service: str = None):
        ...
        if ex and code != 404:
            ...
            self.logger.exception(
                "generic error handler: {}/{}".format(ex.__class__.__name__, str(ex))
            )
