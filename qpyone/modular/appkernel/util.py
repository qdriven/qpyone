...
def extract_model_messages(fileobj, keywords, comment_tags, options):
    ...
    def extract_model():
        ...
        for class_ in classes:
            if has_model(class_):
                for param in [
                    p
                    for p in class_.body
                    if isinstance(p, ast.Assign) and is_parameter(p)
                ]:
                    ...
                    yield (
                        param.lineno,
                        "",
                        "{}.{}".format(clazz_name, parameter_name),
    ...
    return itertools.chain(
        extract_python(fileobj, keywords, comment_tags, options), extract_model()
    )
